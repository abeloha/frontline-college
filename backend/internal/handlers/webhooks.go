...
func RazzWebhookHandler(c *gin.Context) {
	...
	if payload.Event != "virtual_account.paid" || payload.Status != "paid" {
		c.JSON(http.StatusOK, gin.H{"status": true, "message": "noted"})
		return
	}
	...
	if err := db.DB.Where("reference = ?", payload.Reference).First(&va).Error; err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"status": true, "message": "reference not found"})
		return
	}
	...
	if va.Status == models.VAStatusPaid {
		c.JSON(http.StatusOK, gin.H{"status": true, "message": "already processed"})
		return
	}
	...
	if err := db.DB.First(&app, va.ApplicationID).Error; err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"status": true, "message": "application not found"})
		return
	}
	...
	if err := db.DB.Model(&models.VirtualAccount{}).Where("id = ? AND status = ?", va.ID, models.VAStatusPending).
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": "could not update virtual account"})
		return
	}
	...
	if err := db.DB.Create(&proof).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "payment confirmed but could not record proof"})
		return
	}
	...
	if err := db.DB.Model(&models.Application{}).Where("id = ?", app.ID).Update("status", nextStatus).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "payment confirmed but application status update failed"})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"status": true, "message": "payment confirmed"})
}
